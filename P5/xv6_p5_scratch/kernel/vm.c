...
//implement a variant of copyuvm()
pde_t*
cowuvm(pde_t *pgdir, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
    ...
    //convert writeable flags to read-only
    *pte &= ~PTE_W;

    pa = PTE_ADDR(*pte);
    flags = PTE_FLAGS(*pte);

    //increase the reference count of the kernel page
    inc_dec_ReferenceCount((char*)pa, 1);
...
pgflt_handler(void)
{
  ...
  //case 1:the page has more than one reference 
  if(getRefCount((char*)pa) > 1){
    if((mem = kalloc()) == 0)
      panic("CoW: Invalid kalloc");

    //copy the page and replace it with a writeable copy in the local process
    memmove(mem, (char*)pa, PGSIZE);
  ...
  //case2: the page has only one reference
  else if(getRefCount((char*)pa) == 1){
    //restore write permission to the page
    *pte |= PTE_W;
    lcr3(PADDR(proc->pgdir));
    return;
  }
  panic("CoW: Invalid ref count");
