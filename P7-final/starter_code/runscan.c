...
/*
/ Does two main functionalities
/ 1. Reconstruct all jpg files (both undeleted and deleted ones) from a given disk image
/ 2. Find out the filenames of those inodes that represent the jpg files.
*/
void reconstruct_jpg(int fd, char* output_dir)
{
    ...
    for(unsigned int group_i=0; group_i < num_groups; group_i++)
    {
        ...
        for(unsigned int inode_i = first_inode_i; inode_i <= first_inode_i + inodes_per_group; inode_i++)
        {
            ...
            //Part2 start
            //if the inode is about the directory
            ...
            //if the inode is regular file
            else if (S_ISREG(inode->i_mode)) {
                
                //variable that finds jpg file
                int is_jpg = 0;
                
                //read corresponding block; the first block of inode and store them in a buffer
                lseek(fd,BLOCK_OFFSET(inode->i_block[0]),SEEK_SET);
                read(fd,buffer,1024);
                ...
                if(is_jpg==1)
                {
                    ...
                    ////Generate the output file with the correct forat of temporary file name
                    char name_file[1024];
...
//Helper method that write all blocks pointer of inode
void write_block(struct ext2_inode* inode, int fd, char* buffer, int output_file)
{
    ...
    for(int block_i=0; block_i<EXT2_N_BLOCKS; block_i++)
    {
        ...
        if(block_i < EXT2_NDIR_BLOCKS)
        {
            ...
            curr_count_block++;
            //read starting from the block offset of the num_block
            lseek(fd, BLOCK_OFFSET(num_block), SEEK_SET);
            ...
            {
                //allocate with the last offset size
        ...
        else if(block_i == EXT2_NDIR_BLOCKS)
        {
            ...
            //read starting from the block offset of the num_indir_block   
        ...
        else if(block_i == EXT2_DIND_BLOCK)
        {
            ...
            if(num_db_block != 0)
            {
                //read starting from the block offset of the num_db_block 
